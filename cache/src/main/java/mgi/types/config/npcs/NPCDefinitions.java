...
import mgi.tools.parser.Int2ObjectMapDeserializer;
...
public final class NPCDefinitions implements Definitions, Cloneable, TransmogrifiableType, RenderType {
    ...
    public static void dumpToJson(String filePath) throws IOException {
        if (definitions == null) {
            System.err.println("NPCDefinitions not loaded.");
            return;
        }
        Gson gson = new GsonBuilder().setPrettyPrinting().create();

        // Filter out nulls
        NPCDefinitions[] defsToDump = Arrays.stream(definitions)
                .filter(Objects::nonNull)
                .toArray(NPCDefinitions[]::new);

        try (FileWriter writer = new FileWriter(filePath)) {
            gson.toJson(defsToDump, writer);
        }
        System.out.println("Dumped " + defsToDump.length + " NPC definitions to " + filePath);
    }
    public static void loadFromJsonAndPack(String filePath) throws IOException {
        Gson gson = new GsonBuilder()
                .registerTypeAdapter(Int2ObjectMap.class, new Int2ObjectMapDeserializer())
                .create();

        Type listType = new TypeToken<List<NPCDefinitions>>() {}.getType();

        List<NPCDefinitions> npcList;
        try (FileReader reader = new FileReader(filePath)) {
            npcList = gson.fromJson(reader, listType);
        }

        if (npcList == null) {
            System.err.println("No NPC definitions found in JSON.");
            return;
        }

        // Initialize definitions array to max ID + 1 or a fixed size
        int maxId = npcList.stream()
                .filter(Objects::nonNull)
                .mapToInt(NPCDefinitions::getId)
                .max()
                .orElse(0);
        definitions = new NPCDefinitions[maxId + 1];

        for (NPCDefinitions npc : npcList) {
            if (npc == null) continue;
            npc.pack();
        }
        System.out.println("Loaded and packed " + npcList.size() + " NPC definitions from " + filePath);
    }
    ...
    /**
     * Replaces all NPC names equal to "FashionScape" with "OS KO".
     */
    public static void renameFashionScapeToOSKO() {
        if (definitions == null) {
            System.err.println("NPCDefinitions not loaded yet.");
            return;
        }
        int count = 0;
        for (NPCDefinitions def : definitions) {
            if (def == null) continue;

            boolean changed = false;

            // Replace in name
            if ("FashionScape".equals(def.getName())) {
                def.setName("OS KO");
                def.lowercaseName = "os ko";
                changed = true;
            }
            ...
            if (def.getOptions() != null) {
                ...
                for (int i = 0; i < options.length; i++) {
                    if ("FashionScape".equals(options[i])) {
                        options[i] = "OS KO";
                        changed = true;
                    }
                }
            ...
            // Replace in filteredOptions array
            if (def.getFilteredOptions() != null) {
                String[] filteredOptions = def.getFilteredOptions();
                for (int i = 0; i < filteredOptions.length; i++) {
                    if ("FashionScape".equals(filteredOptions[i])) {
                        filteredOptions[i] = "OS KO";
                        changed = true;
                    }
                }
            }
            ...
            if (changed) {
                def.pack(); // Save changes back to cache
                count++;
            }
        }
        System.out.println("Renamed " + count + " NPC(s) from 'FashionScape' to 'OS KO' in all relevant fields.");
    }
