...
//        packMusic();
        ...
        //testInterfaces();
//        final Collection<WorldObject> objs = MapUtils.decode(new ByteBuffer(Files.readAllBytes(Paths.get("assets/Summer/map/l_9810.dat"))));
//        objs.forEach(o -> {
//            set.add(o.getId());
//            o.setId(transformedObjects.getOrDefault(o.getId(), o.getId()));
//        });
//        final Collection<WorldObject> scourgeObjs = MapUtils.decode(new ByteBuffer(Files.readAllBytes(Paths.get("assets/Summer/map/l_9811.dat"))));
//        scourgeObjs.forEach(o -> {
//            set.add(o.getId());
//            o.setId(transformedObjects.getOrDefault(o.getId(), o.getId()));
//        });
//        packMapPre209(8771, Files.readAllBytes(Paths.get("assets/Summer/map/m_9810.dat")), Regions.inject(MapUtils.encode(objs).getBuffer(), o -> false));
//        packMapPre209(8772, Files.readAllBytes(Paths.get("assets/Summer/map/m_9811.dat")), Regions.inject(MapUtils.encode(scourgeObjs).getBuffer(), o -> false));
    }

    public void packInstructionsInterface() throws IOException {
        packModel(53016, Files.readAllBytes(Paths.get("assets/Summer/interfaces/models/712-bottompiece.dat")));
        packModel(53017, Files.readAllBytes(Paths.get("assets/Summer/interfaces/models/712-toppiece.dat")));
        final Cache cache = CacheManager.getCache();
        final Group group = new Group(712);
        for (int i = 0; i < 4; i++) {
            final ComponentDefinitions def = new ComponentDefinitions(712 << 16 | i, new ByteBuffer(Files.readAllBytes(Paths.get("assets/Summer/interfaces/712/" + i + ".dat"))));
    ...
    private static final void packMusic() throws IOException {
        CacheManager.getCache().getArchive(ArchiveType.MUSIC).addGroup(new Group(2502, new mgi.tools.jagcached.cache.File(new ByteBuffer(IOUtils.toByteArray(new FileInputStream(new File("assets/Summer/music/Lazy Wabbit.dat")))))));
        ...
        /*Game.getCacheMgi().getArchive(ArchiveType.MUSIC).addGroup(new Group(2503, new mgi.tools.jagcached.cache.File(new ByteBuffer(IOUtils.toByteArray(new FileInputStream(new File("assets" +
                "/Summer/music/Hare-brained Machines.dat")))))));*/
    }

    public void testInterfaces() throws IOException {
        packModel(53006, Files.readAllBytes(Paths.get("assets/Summer/interfaces/models/32820.dat")));
        packModel(53007, Files.readAllBytes(Paths.get("assets/Summer/interfaces/models/32821.dat")));
        packModel(53008, Files.readAllBytes(Paths.get("assets/Summer/interfaces/models/32825.dat")));
        packModel(53009, Files.readAllBytes(Paths.get("assets/Summer/interfaces/models/32826.dat")));
        packModel(53010, Files.readAllBytes(Paths.get("assets/Summer/interfaces/models/32828.dat")));
        packModel(53011, Files.readAllBytes(Paths.get("assets/Summer/interfaces/models/32830.dat")));
        packModel(53012, Files.readAllBytes(Paths.get("assets/Summer/interfaces/models/32829.dat")));
        packModel(53013, Files.readAllBytes(Paths.get("assets/Summer/interfaces/models/32824.dat")));
        packModel(53014, Files.readAllBytes(Paths.get("assets/Summer/interfaces/models/32822.dat")));
        packModel(53015, Files.readAllBytes(Paths.get("assets/Summer/interfaces/models/32823.dat")));
        ...
        for (int id = 10820; id <= 10832; id++) {
            packClientScript(id, Files.readAllBytes(Paths.get("assets/Summer/clientscripts/" + id)));
        }
        final Cache cache = CacheManager.getCache();
        ...
        map.put(537, 53016);
        for (int i = 0; i < 31; i++) {
            final ComponentDefinitions def = new ComponentDefinitions(711 << 16 | i, new ByteBuffer(Files.readAllBytes(Paths.get("assets/Summer/interfaces/711/" + i + ".dat"))));
    ...
    public void packObjects() throws IOException {
        Iterator<File> it = FileUtils.iterateFiles(new File("assets/Summer/object definitions/"), null, false);
    ...
    public void packModels() throws IOException {
        Iterator<File> it = FileUtils.iterateFiles(new File("assets/Summer/models/"), null, true);
        ...
        for (final Integer originalId : list) {
            File file = new File("assets/Summer/models/" + originalId + ".dat");
            if (!file.exists()) {
                file = new File("assets/Summer/models/extra/" + originalId + ".dat");
            }
    ...
    private final void packSoundEffects() throws IOException {
        Iterator<File> it = FileUtils.iterateFiles(new File("assets/Summer/sounds/"), null, false);
        final Int2ObjectAVLTreeMap<File> sortedMap = new Int2ObjectAVLTreeMap<File>();
        while (it.hasNext()) {
            final File file = it.next();
            final int originalId = Integer.parseInt(file.getName().replace(".dat", ""));
    ...
    public void packVarbits() throws IOException {
        Iterator<File> it = FileUtils.iterateFiles(new File("assets/Summer/varbit definitions/"), null, true);
        final Int2ObjectAVLTreeMap<File> sortedMap = new Int2ObjectAVLTreeMap<File>();
    ...
    public void packGraphics() throws IOException {
        Iterator<File> it = FileUtils.iterateFiles(new File("assets/Summer/graphic definitions/"), null, false);
        final Int2ObjectAVLTreeMap<File> sortedMap = new Int2ObjectAVLTreeMap<File>();
        while (it.hasNext()) {
            final File file = it.next();
            final int originalId = Integer.parseInt(file.getName().replace(".dat", ""));
    ...
    private void packAnimationBase(@NotNull final AnimationBase base, final int[] anims) throws IOException {
        for (int anim : anims) {
            final File file = new File("assets/Summer/animation definitions/" + anim + ".dat");
            ...
            if (OUTPUT) {
                System.err.println("Summer anim trans: " + anim + ", " + definitions.getId());
            }
    ...
    private final void packFrames(final int[] ids) {
        ...
        for (int id : ids) {
            try {
                File file = new File("assets/Summer/animation frames/" + id + ".dat");
    ...
    public void packNPCs() throws IOException {
        Iterator<File> it = FileUtils.iterateFiles(new File("assets/Summer/npc definitions/"), null, true);
        final Int2ObjectAVLTreeMap<File> sortedMap = new Int2ObjectAVLTreeMap<File>();
        while (it.hasNext()) {
            final File file = it.next();
            final int originalId = Integer.parseInt(file.getName().replace(".dat", ""));
...
//        }
    ...
    public void preloadItems() {
        ...
        Iterator<File> it = FileUtils.iterateFiles(new File("assets/Summer/item definitions/"), null, false);
        while (it.hasNext()) {
            final File file = it.next();
            final int originalId = Integer.parseInt(file.getName().replace(".dat", ""));
