...
object DeveloperCommands {
    ...
    fun register() {
        ...
        // Inside DeveloperCommands.register() function, add:

        Command(PlayerPrivilege.DEVELOPER, "packgraphics") { player, _ ->
            try {
                TypeParser.packSprites()
                TypeParser.packClientBackground()
                player.sendMessage("Successfully packed sprites and client background.")
            } catch (e: IOException) {
                player.sendMessage("Failed to pack sprites or client background: ${e.message}")
                e.printStackTrace()
            }
        }
